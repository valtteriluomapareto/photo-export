...
enum AutoSyncState: Equatable, Sendable {
  ...
  /// A run is in flight. The reducer carries the trigger `reason` only — the actual
  /// `ExportRunContext.runId` lives on `state.exportRunState.activeContext` and is
  /// generated by the effect runner at dispatch time, so the pure reducer never has
  /// to mock UUID generation. Returns to `.idle` when the matching
  /// `exportRunStateChanged(.idle)` arrives.
  case running(reason: AutoSyncReason)
}
