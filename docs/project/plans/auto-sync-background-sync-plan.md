...
Date: 2026-05-08
Status: In progress (last status update 2026-05-09)
...
| Phase | Status | Notes |
|-------|--------|-------|
| **Phase 0a** Pure Refactor Foundations | ✅ Complete | Awaitable `runExport`, fingerprint identity, lifecycle coordinator, destination loss interruption. Branch `auto-sync-phase-0a` pushed. |
| **Phase 0b** Locking and Safety | ❌ Not started | Gated by the advisory-lock spike. Destination safety scan (`unsafeNeedsConfirmation`) also deferred — currently only `unsafeMigrationConflict` is populated. |
...
| **Phase 3** Retry and Run Policy | 🟡 Slice A only | **Done:** `manualFullExportCompleted` clears compatible dirty (reducer + dispatch wiring); `retryTimerFired` falls through to recompute. **Missing:** failure-category routing (Slice B), retry-eligibility at enqueue (Slice C), manual retry override. |
| **Phase 4** UI | 🟡 Slice 1 only | **Done:** Settings → Auto Export tab (enable + scopes + status + Export Now), toolbar status pill, `runNow()` API. **Missing:** menu bar item (NSStatusItem), notifications, Dock badge, Export Issues tab, `Open at login`, first-toggle-on hand-off, manual-export confirmation sheet, VoiceOver labels. |
...
Branch tips: `auto-sync-phase-0a` (Phase 0a), `auto-sync-phase-2` (adds Phase 1 + 2), `auto-sync-phase-3` (adds Phase 3 Slice A), `auto-sync-phase-4` (adds Phase 4 Slice 1; cumulative tip).
