...
- **ExportDestinationManager** — manages the chosen export destination folder (security-scoped bookmarks) and owns the **stable logical destination id** that keys every per-destination store. Publishes identity (stable id + advisory fingerprint) as one atomic `@Published` value. See [`docs/reference/architecture-conventions.md`](docs/reference/architecture-conventions.md) §Destination identity.
- **ExportRecordStore** — tracks timeline (year/month) exports per-destination (keyed by the stable id). Reconfigures when destination changes.
...
- **Stable logical destination id, not the fingerprint, is the keying id.** Per-destination state used to be keyed on `DestinationFingerprint.id` (SHA-256 of volume UUID + path, or canonical path for volumes without a UUID). On a network share that has no volume UUID, the path-based fingerprint drifts across remount, so the app opened an empty record dir and re-exported everything (#127). The fix: `ExportDestinationManager` seeds a stable id from the fingerprint once, persists it beside the bookmark, and reuses it verbatim while the bookmark resolves to the same folder; the fingerprint is demoted to the AutoSync safety gate's confidence signal. A simpler "recompute each launch" or "mint a new id when the path differs" both re-introduce the bug — the latter through the picker on a re-grant. Full contract + the CI keying guard: [`docs/reference/architecture-conventions.md`](docs/reference/architecture-conventions.md) §Destination identity.
