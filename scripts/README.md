...
| [`screenshots/capture.sh`](screenshots/capture.sh) | Builds the app in screenshot mode (curated synthetic library, no personal photos), launches it at a requested window size, and drives it through marketing surfaces via [`screenshots/drive.applescript`](screenshots/drive.applescript). Outputs to `screenshots/<WxH>/NN-name.png`. Requires one-time Automation + Screen Recording TCC grants — see the script header. See [`docs/project/plans/screenshot-automation-plan.md`](../docs/project/plans/screenshot-automation-plan.md) for design. |
