...
struct AutoSyncManagerTests {
  ...
  // MARK: - PHAsset cache draining between sub-scopes (issue #112)

  /// Each sub-scope iteration drains `phAssetCache` after `runExport`
  /// returns, before moving to the next iteration. Pins the structural
  /// behaviour required to keep the fan-out's peak memory bounded by one
  /// scope's working set rather than the cumulative timeline + favorites +
  /// every album + shared-album sweep. The recording fake (`RecordingPHAssetCacheControl`)
  /// counts invocations rather than instrumenting actual memory, mirroring
  /// the `RecordingDirectoryFsync` pattern from PR #114 — fast, deterministic,
  /// and a future refactor that drops the call would otherwise pass every
  /// other test and silently re-introduce the OOM kill on large libraries.
  @Test func fanOutCallsForgetPHAssetCacheAfterEachScope() async {
    let manager = AutoSyncManager()
    let builder = FakeAutoSyncEnvironmentBuilder()
    builder.userDefaults.set(true, forKey: AutoSyncManager.enabledDefaultsKey)
    builder.destination.subject.send(safeDestination())
    ...
    #expect(
      builder.phAssetCacheControl.forgetCallCount == 3,
      "fan-out must call forgetPHAssetCache() once per sub-scope (timeline + favorites + albums)"
    )
  }

  /// `forgetPHAssetCache()` runs *before* the `summary.result != .completed`
  /// short-circuit. Pins the unconditional-drain behaviour: even a scope
  /// that fails or is cancelled has already added its fetches to the cache,
  /// so dropping is required regardless of outcome.
  @Test func fanOutCallsForgetPHAssetCacheBeforeBreakingOnNonCompletedScope() async {
    let manager = AutoSyncManager()
    let builder = FakeAutoSyncEnvironmentBuilder()
    builder.userDefaults.set(true, forKey: AutoSyncManager.enabledDefaultsKey)
    builder.destination.subject.send(safeDestination())
    ...
    // First scope returns a non-`.completed` summary so the fan-out breaks
    // after the first iteration. The forget call must still have happened
    // for that one iteration.
    let firstContext = ExportRunContext(
    ...
    builder.exportRunner.nextRunSummary = ExportRunSummary(
      context: firstContext, endedAt: Date(),
      enqueuedCount: 0, completedCount: 0, failedCount: 1, skippedCount: 0,
    ...
    #expect(
      ...
      "fan-out must break after the first non-completed scope")
    #expect(
      builder.phAssetCacheControl.forgetCallCount == 1,
      "forgetPHAssetCache() must still run for the failed scope — its fetches accumulated in the cache before the failure")
  }
